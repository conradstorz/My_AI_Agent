...
def analyze_with_openai(content: str, filename: str) -> dict:
    ...
    ---
    {content[:3500]} # Truncate to avoid token overrun
    ---
    ...
    try:
        ...
        return {"summary": content, "contains_structured_data": None, "success": False, "error": "invalid JSON"}
    except Exception as e:
        logger.error(f"OpenAI error: {e}")
        return {"summary": None, "contains_structured_data": None, "success": False, "error": str(e)}
...
def analyze_file(file: Path):
    ...
    result = analyze_with_openai(content, file.name)
    result.update({
        "filename": file.name,
        "timestamp": time.time(),
